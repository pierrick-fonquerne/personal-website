...
<Callout kind="tip" title="In practice: dot product or cosine?">
If all your vectors are normalized (length one), the dot product and the cosine become identical, because the denominator of the cosine formula equals one. Many embedding models already return normalized vectors. In that case, you can use the dot product directly: it skips the division by the norms, so it is faster to compute, while you are in effect reasoning about angles. Keep the explicit cosine or Euclidean distance when vectors are not normalized, or when the length carries information you want to preserve. The rule to remember: normalize once, then the dot product is enough.
</Callout>
...
- Jurafsky, D. & Martin, J. H. (2024). _Speech and Language Processing_ (3rd edition draft), ch. 6 "Vector Semantics and Embeddings". A reference textbook, accessible and free online. [web.stanford.edu/~jurafsky/slp3](https://web.stanford.edu/~jurafsky/slp3/)
