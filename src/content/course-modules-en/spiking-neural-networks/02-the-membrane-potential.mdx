...
<Callout kind="note" title="One quantity, two causes">
  Since $\tau = RC$, two very different membranes can share the same clock. Doubling the resistance
  $R$ or doubling the capacitance $C$ produces the same lengthening of $\tau$, hence the same
  slowness of leak. The time constant alone does not tell you whether the membrane is very resistive
  or very capacitive: it keeps only their product.
</Callout>
...
<Callout kind="note" title="The limit of the capacitor analogy">
  The RC holds as long as the membrane stays passive, far from the threshold. Near the firing of the
  spike, the voltage-dependent channels take over and the linear model no longer holds. The RC is
  not wrong then, it is incomplete: it is the foundation on which the following chapters add just
  enough nonlinearity, without paying the cost of the full Hodgkin-Huxley model.
</Callout>
