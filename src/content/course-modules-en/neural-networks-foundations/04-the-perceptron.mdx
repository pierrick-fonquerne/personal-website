...
*"H is the set of points x in R to the n such that the dot product of w with x, plus b, equals zero."* The vector $w$ gives the direction perpendicular to $\mathcal{H}$. The scalar $b$ controls how far $\mathcal{H}$ sits from the origin.
</Callout>
...
If you double both $w$ and $b$, the hyperplane does not move geometrically, but the quantity $w \cdot x + b$ is doubled too. To get a distance that depends only on the hyperplane and not on the particular encoding $(w, b)$ we picked, we normalise by $\|w\|$.
</Callout>
...
*"The new weight vector w is the old w plus the step eta times the target y i times the sample x i. The new bias b is the old plus eta times y i."* The update is therefore an *addition* (or subtraction) of the vector $x_i$ into the vector $w$, signed by $y_i$.
</Callout>
...
To lighten the proof, we absorb the bias into the weights: each $x_i \in \mathbb{R}^n$ is replaced by $\tilde x_i = (x_i, 1) \in \mathbb{R}^{n+1}$, and each $(w, b)$ by $\tilde w = (w, b) \in \mathbb{R}^{n+1}$. The learning rule and the separation inequality keep exactly the same shape, so we can now reason as if the bias did not exist, in a space of dimension $n+1$. Exercise 4 shows why we cannot simply *freeze* $b = 0$ instead: without the absorption, convergence is no longer guaranteed.
</Callout>
...
A naive idea: converge by averaging the $w_t$ obtained at each iteration. That works too (it is the *voted perceptron* of Freund and Schapire (1999)), but it requires keeping a history and weighting each $w_t$ by its lifetime. The Pocket Algorithm is simpler: a single $(w, b)$ stored, updated only when we do strictly better.
</Callout>
...
Marvin <Term slug="minsky-papert">Minsky and Seymour Papert</Term> publish *Perceptrons* in 1969, the central chapter of which proves this impossibility and generalises it to a whole family of "non-local" functions. Their rigorous analysis contributed to a withdrawal of public funding for neural network research, a period we now call the first <Term slug="hiver-de-l-ia">AI winter</Term>. Recovery had to wait for Hopfield in 1982 and the rediscovery of <Term slug="backpropagation">backpropagation</Term> by Rumelhart, Hinton and Williams in 1986.
...
**Step 3.** Check each point. Compute $z = w_1 x_1 + w_2 x_2 + b$ and $\operatorname{sgn}(z)$.
  ...
  questions={[
    {
      ...
      answer: 1,
      explain: "The rule w ← w + η y x is a projection: it adds (or subtracts) the input vector. As proved earlier, it strictly increases the functional margin of the corrected sample, without ever evaluating the slope of a loss function."
    },
    {
      ...
      answer: 1,
      explain: "Novikoff's theorem does not apply. On every correction, another sample becomes misclassified, so the algorithm spends its time fixing one then the other, never converging."
    },
    {
      ...
      answer: 1,
      explain: "If xᵢ = 0, the weight wᵢ did not influence the prediction (multiplied by zero). Correcting it would be arbitrary. The rule adjusts only the responsible weights, proportionally to their input."
    },
    {
      ...
      answer: 1,
      explain: "The hyperplane passes through the points x such that w · x = -b. The signed distance from the origin to the hyperplane is b / ‖w‖. Without bias, the hyperplane is forced through the origin, which severely restricts the family of classifiers we can realise."
    },
    {
      ...
      answer: 1,
      explain: "We proved in this chapter that the four XOR constraints are incompatible. But the decomposition XOR = OR ∧ ¬AND is made entirely of linearly separable functions. Three stacked perceptrons solve XOR: that is the idea of chapter 5."
    }
  ]}
/>
...
- Gallant, S. I. (1990). "Perceptron-Based Learning Algorithms." *IEEE Transactions on Neural Networks* 1(2), 179-191. (Introduces the Pocket Algorithm.) [DOI 10.1109/72.80230](https://doi.org/10.1109/72.80230)
