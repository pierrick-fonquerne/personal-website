...
Your brain contains roughly **86 billion neurons** (Azevedo et al., 2009). Each neuron receives electrical signals from other neurons through its _dendrites_, integrates them in its cell body, and decides, based on an internal threshold, whether to fire a signal along its _axon_.
...
<Formula caption="Expanded form">y = f( x₁·w₁ + x₂·w₂ + x₃·w₃ + b )</Formula>
...
<Callout kind="tip" title="Read the equation out loud">
  The most useful version for training yourself to read maths: *"y equals f applied to the sum, for
  i going from one to n, of the products w i times x i, plus b."* That spoken sentence should become
  a reflex. When you meet a more complex equation later, you will read it the same way, piece by
  piece.
</Callout>
...
<Callout kind="note">
  The artificial neuron is a caricature of the biological neuron. A real neuron is dynamic over
  time, communicates with discrete spikes, and has a complex internal chemistry. Those differences
  fall outside this course, which focuses on the models that dominate practice. They are an active
  research topic in neuromorphic computing and spiking neural networks.
</Callout>
...
| 1969 | **Minsky & Papert** publish _Perceptrons_ and prove a single neuron cannot learn <Term slug="xor">XOR</Term>. First major dent in the field's enthusiasm.                         |
...
A hyperplane splits space into two **half-spaces**: one where $\mathbf{w} \cdot \mathbf{x} + b > 0$ (the neuron _fires_) and one where $\mathbf{w} \cdot \mathbf{x} + b < 0$ (it does not). A threshold neuron is exactly this partition operation.
...
<Callout kind="warn" title="The historical trap">
  In 1969, Minsky and Papert prove that limit formally. Many conclude, wrongly, that neural networks
  are a dead end. Funding collapses. Yet the solution, **multi-layer** networks, already existed in
  theory. We had to wait until 1986 and backpropagation to train them effectively. Subject of
  chapters 5 and 7.
</Callout>

## The role of the bias, visually

Without bias, the line drawn by the neuron must pass through the origin. That is a strong constraint: most real-world problems have a decision boundary that does _not_ sit at the origin.
...
- Lighthill, J. (1973). _Artificial Intelligence: A General Survey_. Science Research Council, United Kingdom. [Archive.org](https://archive.org/details/lighthill-report)

### Further reading

- Goodfellow, I., Bengio, Y. & Courville, A. (2016). _Deep Learning_. MIT Press. Chapter 6 on feed-forward networks. [deeplearningbook.org](https://www.deeplearningbook.org/)
- Bishop, C. M. (2006). _Pattern Recognition and Machine Learning_. Springer. Chapter 5 on neural networks.
- Hastie, T., Tibshirani, R. & Friedman, J. (2009). _The Elements of Statistical Learning_. Springer. Chapter 11. [Free PDF at Stanford](https://hastie.su.domains/ElemStatLearn/)
    ...
    questions={[
      {
        ...
        answer: 1,
        explain:
          'The neuron computes z = Σ wᵢ xᵢ + b, a linear combination, not a product or an average.',
      },
      {
        ...
        answer: 2,
        explain:
          "A neuron draws a line. XOR's positive cases sit diagonally, so no single line can separate them from the negatives.",
      },
      {
        ...
        answer: 1,
        explain:
          "Weights set the line's orientation, the bias sets its position. Without bias, the line must pass through the origin.",
      },
      {
        ...
        answer: 1,
        explain:
          'f squashes or reshapes the weighted sum. Without it, stacking neurons collapses to a linear combination, and a deep network loses its expressive power.',
      },
      {
        ...
        answer: 1,
        explain:
          'The artificial neuron borrows the idea of weighted inputs and a threshold, but ignores the temporal dimension, discrete spikes, and synaptic chemistry. It is a useful caricature, not a replica.',
      },
    ]}
  />
</InteractivePrint>
