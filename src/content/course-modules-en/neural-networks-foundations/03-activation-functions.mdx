...
<Callout kind="tip" title="The theorem in one sentence">
  *Without non-linearity, network depth is an illusion.* That is why we insert a function $f$ at the
  output of every neuron, to "break" linearity. Without it, you do not have a deep network, you have
  a linear regression with fancy notation.
</Callout>
...
<Formula caption="Sigmoid">σ(x) = 1 / (1 + e⁻ˣ)</Formula>
...
<Term slug="relu">ReLU</Term> is today the default choice for hidden layers. Its brutal simplicity
is precisely what makes it efficient.
...
**Modern usage**: since its introduction for RBMs by Nair and Hinton (2010), ReLU and its variants (Leaky ReLU, ELU, GELU) have become the dominant choice for hidden layers in deep networks. It computes in one operation, its gradient is exact (1 or 0), and it does not saturate upwards. Drawback: if a neuron's input stays negative, its gradient is zero and backpropagation (chapter 8) can no longer update its weights. The neuron freezes. This is the _dying ReLU_ problem, mitigated by the variants.
...
<Callout kind="warn" title="The dying ReLU trap">
  If during training you notice that some of your ReLU neurons stay silent across the entire
  dataset, they are dead: their output stays at zero and their gradient too, so backpropagation
  cannot revive them. First reflexes: lower the learning rate (an oversized step can push a neuron
  past a very negative bias in one go), replace the ReLUs with **Leaky ReLU** or **GELU** (variants
  that let some gradient through on the negative side), and check the bias initialisation (chapter
  11), which should not be overly negative.
</Callout>
...
ReLU has been the default since 2012, but state-of-the-art architectures (transformers, foundation models, diffusion) have adopted smoother variants that solve the _dying ReLU_ problem while preserving most of its advantages.
...
<Formula caption="ELU">ELU(x) = x if x ≥ 0, else α(eˣ - 1)</Formula>
...
<Callout kind="warn" title="The vanishing-gradient trap">
  If you train a deep network and the first layer stays mute while the last one converges, you
  almost certainly have a vanishing gradient. First reflexes: swap sigmoids for ReLUs, check the
  initialisation (chapter 11), and try batch normalisation (same chapter).
</Callout>
...
- Nair, V. & Hinton, G. E. (2010). "Rectified Linear Units Improve Restricted Boltzmann Machines." _ICML_ 27. [ICML link](https://icml.cc/Conferences/2010/papers/432.pdf)
- Glorot, X. & Bengio, Y. (2010). "Understanding the difficulty of training deep feedforward neural networks." _AISTATS_ 9, 249-256. [AISTATS link](https://proceedings.mlr.press/v9/glorot10a.html)
- Cox, D. R. (1958). "The Regression Analysis of Binary Sequences." _Journal of the Royal Statistical Society_ 20(2), 215-242. (Historical origin of the logistic function.) [DOI 10.1111/j.2517-6161.1958.tb00292.x](https://doi.org/10.1111/j.2517-6161.1958.tb00292.x)

### Further reading

- Goodfellow, I., Bengio, Y. & Courville, A. (2016). _Deep Learning_. MIT Press. Section 6.3 on activation functions and their variants. [deeplearningbook.org](https://www.deeplearningbook.org/)
- Maas, A. L., Hannun, A. Y. & Ng, A. Y. (2013). "Rectifier Nonlinearities Improve Neural Network Acoustic Models." _ICML_ 30, Workshop on Deep Learning for Audio, Speech and Language Processing. (Introduction of Leaky ReLU.) [Stanford link](https://ai.stanford.edu/~amaas/papers/relu_hybrid_icml2013_final.pdf)
- Hendrycks, D. & Gimpel, K. (2016). "Gaussian Error Linear Units (GELUs)." _arXiv_. [arXiv 1606.08415](https://arxiv.org/abs/1606.08415)
- Ramachandran, P., Zoph, B. & Le, Q. V. (2017). "Searching for Activation Functions." _arXiv_. (Automated search for activations, introduces the Swish family, equivalent to SiLU.) [arXiv 1710.05941](https://arxiv.org/abs/1710.05941)
- Elfwing, S., Uchibe, E. & Doya, K. (2018). "Sigmoid-Weighted Linear Units for Neural Network Function Approximation in Reinforcement Learning." _Neural Networks_ 107, 3-11. (Original description of SiLU.) [arXiv 1702.03118](https://arxiv.org/abs/1702.03118)
    ...
    questions={[
      {
        ...
        answer: 1,
        explain:
          'Without non-linearity, a composition of affine layers stays affine: y = W₂(W₁x + b₁) + b₂ = (W₂W₁)x + (W₂b₁ + b₂). Depth brings nothing mathematically.',
      },
      {
        ...
        answer: 2,
        explain:
          'An elegant property of the sigmoid: its derivative is expressible directly from its own value. That simplifies backpropagation since we do not have to recompute the exponential.',
      },
      {
        ...
        answer: 1,
        explain:
          'Ultra-fast computation (max with 0) and exact gradient of 1 on the active part. ReLU is not bounded above, not probabilistic, and not strictly differentiable at 0, but those points are negligible in practice.',
      },
      {
        ...
        answer: 2,
        explain:
          'Sigmoid squashes the output into ]0, 1[, giving a directly interpretable probability. For multi-class classification we would use softmax (chapter 6).',
      },
      {
        ...
        answer: 1,
        explain:
          "Sigmoid's derivative has a maximum of 0.25. Multiplied 10 times, you get 0.25^10 ≈ 9.5×10⁻⁷. The input-side gradient collapses, the first layer stops learning. That is why we use ReLU in deep hidden layers.",
      },
    ]}
  />
</InteractivePrint>
