...
<Callout kind="tip" title="Le théorème en une phrase">
  *Sans non-linéarité, la profondeur d'un réseau est une illusion.* C'est pour ça qu'à chaque sortie
  de neurone, on insère une fonction $f$ qui « casse » la linéarité. Sans elle, tu n'as pas un
  réseau profond, tu as une régression linéaire avec une notation compliquée.
</Callout>
...
<Formula caption="Identity">f(x) = x</Formula>
...
La <Term slug="sigmoide">sigmoïde</Term> a dominé les réseaux de neurones entre 1986 et 2010. Sa forme en S compresse n'importe quel réel dans l'intervalle ouvert $]0, 1[$, ce qui permet d'interpréter sa sortie comme une probabilité.

<Formula caption="Sigmoïde">σ(x) = 1 / (1 + e⁻ˣ)</Formula>
...
**Usage moderne** : couche de sortie des classifications binaires (où la sortie probabilité a un sens). Plus utilisée pour les couches cachées profondes à cause du _vanishing gradient_ (on y revient juste après).
...
<Formula caption="ReLU">ReLU(x) = max(0, x)</Formula>
...
**Usage moderne** : depuis son introduction pour les RBM par Nair et Hinton (2010), ReLU et ses variantes (Leaky ReLU, ELU, GELU) sont devenues le choix dominant pour les couches cachées des réseaux profonds. Elle calcule en une seule opération, son gradient est exact (1 ou 0), et elle ne sature pas vers le haut. Inconvénient : si l'entrée d'un neurone est constamment négative, son gradient est nul et la backpropagation (chapitre 8) ne peut plus mettre à jour ses poids. Le neurone se fige. C'est le problème du _dying ReLU_, qu'on contourne avec les variantes.
...
<Callout kind="warn" title="Le piège du dying ReLU">
  Si tu observes en cours d'entraînement qu'une partie de tes neurones ReLU restent muets sur
  l'ensemble du dataset, ils sont morts : leur sortie reste à zéro et leur gradient aussi, donc la
  backpropagation ne peut plus les ressusciter. Premiers réflexes : réduis le taux d'apprentissage
  (un pas trop grand peut faire basculer un neurone du bon côté à un biais très négatif d'un coup),
  remplace les ReLU par des **Leaky ReLU** ou **GELU** (variantes qui laissent passer un peu de
  gradient sur la partie négative), et vérifie l'initialisation des biais (chapitre 11) qui ne doit
  pas être trop négative.
</Callout>
...
<Formula caption="Tanh">tanh(x) = (eˣ - e⁻ˣ) / (eˣ + e⁻ˣ)</Formula>
...
ReLU est le choix par défaut depuis 2012, mais les architectures de pointe (transformers, modèles de fondation, diffusion) ont adopté des variantes plus douces qui résolvent le _dying ReLU_ tout en gardant l'essentiel des avantages.

### Leaky ReLU

<Formula caption="Leaky ReLU">LeakyReLU(x) = max(α x, x)</Formula>
...
<Formula caption="GELU">GELU(x) = x · Φ(x)</Formula>
...
<Formula caption="SiLU / Swish">SiLU(x) = x · σ(x)</Formula>
...
<Formula caption="ELU">ELU(x) = x si x ≥ 0, sinon α(eˣ - 1)</Formula>
...
<Callout kind="warn" title="Le piège du vanishing gradient">
  Si tu entraînes un réseau profond et que la première couche reste muette alors que la dernière
  converge, c'est très probablement un vanishing gradient. Premiers réflexes : remplace les
  sigmoïdes par des ReLU, vérifie l'initialisation (chapitre 11), et regarde si la batch
  normalisation aide (même chapitre).
</Callout>
...
| ------------------------------------- | ---------------------------------- | -------------------------------------------------- |
| Couches cachées                       | **ReLU**                           | Leaky ReLU, ELU, GELU pour les cas de _dying ReLU_ |
...
- Nair, V. & Hinton, G. E. (2010). « Rectified Linear Units Improve Restricted Boltzmann Machines. » _ICML_ 27. [Lien ICML](https://icml.cc/Conferences/2010/papers/432.pdf)
- Glorot, X. & Bengio, Y. (2010). « Understanding the difficulty of training deep feedforward neural networks. » _AISTATS_ 9, 249-256. [Lien AISTATS](https://proceedings.mlr.press/v9/glorot10a.html)
- Cox, D. R. (1958). « The Regression Analysis of Binary Sequences. » _Journal of the Royal Statistical Society_ 20(2), 215-242. (Origine historique de la fonction logistique). [DOI 10.1111/j.2517-6161.1958.tb00292.x](https://doi.org/10.1111/j.2517-6161.1958.tb00292.x)

### Pour aller plus loin

- Goodfellow, I., Bengio, Y. & Courville, A. (2016). _Deep Learning_. MIT Press. Section 6.3 sur les fonctions d'activation et leurs variantes. [deeplearningbook.org](https://www.deeplearningbook.org/)
- Maas, A. L., Hannun, A. Y. & Ng, A. Y. (2013). « Rectifier Nonlinearities Improve Neural Network Acoustic Models. » _ICML_ 30, Workshop on Deep Learning for Audio, Speech and Language Processing. (Introduction de la Leaky ReLU). [Lien Stanford](https://ai.stanford.edu/~amaas/papers/relu_hybrid_icml2013_final.pdf)
- Hendrycks, D. & Gimpel, K. (2016). « Gaussian Error Linear Units (GELUs). » _arXiv_. [arXiv 1606.08415](https://arxiv.org/abs/1606.08415)
- Ramachandran, P., Zoph, B. & Le, Q. V. (2017). « Searching for Activation Functions. » _arXiv_. (Recherche automatisée d'activations, introduit la famille Swish, équivalente à SiLU). [arXiv 1710.05941](https://arxiv.org/abs/1710.05941)
- Elfwing, S., Uchibe, E. & Doya, K. (2018). « Sigmoid-Weighted Linear Units for Neural Network Function Approximation in Reinforcement Learning. » _Neural Networks_ 107, 3-11. (Description originale de SiLU). [arXiv 1702.03118](https://arxiv.org/abs/1702.03118)
- Clevert, D.-A., Unterthiner, T. & Hochreiter, S. (2015). « Fast and Accurate Deep Network Learning by Exponential Linear Units (ELUs). » _arXiv_. [arXiv 1511.07289](https://arxiv.org/abs/1511.07289)
    ...
    questions={[
      {
        ...
        answer: 1,
        explain:
          "Sans non-linéarité, une composition de couches affines reste affine : y = W₂(W₁x + b₁) + b₂ = (W₂W₁)x + (W₂b₁ + b₂). La profondeur du réseau n'apporte rien mathématiquement.",
      },
      {
        q: 'Quelle est la dérivée de la sigmoïde σ(x) ?',
        options: ['σ(x)', '1 - σ(x)', 'σ(x)(1 - σ(x))', '1 / (1 + e⁻ˣ)'],
        answer: 2,
        explain:
          "Propriété élégante de la sigmoïde : sa dérivée s'exprime directement à partir de sa valeur. Cela simplifie la backpropagation puisqu'on n'a pas à recalculer l'exponentielle.",
      },
      {
        ...
        answer: 1,
        explain:
          "Calcul ultra-rapide (max avec 0) et gradient exact de 1 sur la partie active. ReLU n'est pas bornée vers le haut, n'est pas probabiliste, et n'est pas strictement dérivable en 0, mais ces points sont négligeables en pratique.",
      },
      {
        q: "Sur une classification binaire, quelle fonction d'activation utilise-t-on typiquement en sortie ?",
        options: ['ReLU', 'Tanh', 'Sigmoïde', 'Identity'],
        answer: 2,
        explain:
          'La sigmoïde compresse la sortie dans ]0, 1[, ce qui donne une probabilité directement interprétable. Pour de la classification multi-classe, on utiliserait softmax (chapitre 6).',
      },
      {
        ...
        answer: 1,
        explain:
          "La dérivée de sigmoïde a un maximum de 0,25. Multiplié 10 fois, on obtient 0,25^10 ≈ 9,5×10⁻⁷. Le gradient à l'entrée s'écroule, la première couche n'apprend plus. C'est pour ça qu'on utilise ReLU sur les couches cachées profondes.",
      },
    ]}
  />
</InteractivePrint>
