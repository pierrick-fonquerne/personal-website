...
Si tu observes en cours d'entraînement qu'une partie de tes neurones ReLU restent muets sur l'ensemble du dataset, ils sont morts : leur sortie reste à zéro et leur gradient aussi, donc la backpropagation ne peut plus les ressusciter. Premiers réflexes : réduis le taux d'apprentissage (un pas trop grand peut faire basculer un neurone du bon côté à un biais très négatif d'un coup), remplace les ReLU par des **Leaky ReLU** ou **GELU** (variantes qui laissent passer un peu de gradient sur la partie négative), et vérifie l'initialisation des biais (chapitre 11) qui ne doit pas être trop négative.
</Callout>
...
| Tanh | $\tanh(x) = \dfrac{e^x - e^{-x}}{e^x + e^{-x}}$ | $\tanh'(x) = 1 - \tanh^2(x)$ | $(-1, 1)$ | ~15× (deux exp) |
...
**Équation** : $\text{GELU}(x) = x \cdot \Phi(x)$, où $\Phi$ est la fonction de répartition de la loi normale standard. C'est l'activation utilisée dans BERT, GPT-2, GPT-3, GPT-4, Llama, Claude et la plupart des transformers modernes. **Dérivée** : $\Phi(x) + x \cdot \phi(x)$ où $\phi$ est la densité gaussienne.
...
**Équation** : $\text{SiLU}(x) = x \cdot \sigma(x)$, où $\sigma$ est la sigmoïde. C'est la même chose que la fonction « Swish » de Ramachandran et al. (2017), simplement renommée. Utilisée dans EfficientNet, Mixtral, certaines variantes de Llama. **Dérivée** : $\sigma(x) + x \cdot \sigma(x)(1 - \sigma(x))$.
...
>
  <VanishingGradientSimulator
    ...
    labels={{
      ...
      layerPrefix: 'Couche',
      helpText: "La couche de sortie reçoit un gradient de référence de 1. Chaque couche traversée vers l'entrée le multiplie par la dérivée maximale de la fonction choisie : sigmoïde 0,25, tanh 1, ReLU 1.",
    }}
  />
</InteractivePrint>
...
- Ramachandran, P., Zoph, B. & Le, Q. V. (2017). « Searching for Activation Functions. » *arXiv*. (Recherche automatisée d'activations, introduit la famille Swish, équivalente à SiLU). [arXiv 1710.05941](https://arxiv.org/abs/1710.05941)
- Elfwing, S., Uchibe, E. & Doya, K. (2018). « Sigmoid-Weighted Linear Units for Neural Network Function Approximation in Reinforcement Learning. » *Neural Networks* 107, 3-11. (Description originale de SiLU). [arXiv 1702.03118](https://arxiv.org/abs/1702.03118)
