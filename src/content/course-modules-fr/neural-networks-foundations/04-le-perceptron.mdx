...
Pose une règle plate sur une table. Cette règle divise la surface de la table en deux zones : la partie devant la règle et la partie derrière la règle. Le bord de la règle, c'est la _frontière_. La direction perpendiculaire au bord, c'est ce qu'on va appeler le <Term slug="vecteur-normal">vecteur normal</Term>, et la distance entre la règle et le bord de la table, c'est le décalage.
...
<Callout kind="tip" title="Lis l'équation à voix haute">
  *« H est l'ensemble des points x de R puissance n tels que le produit scalaire de w par x, plus b,
  est égal à zéro. »* Le vecteur $w$ donne la direction perpendiculaire à $\mathcal{H}$. Le scalaire
  $b$ règle la position de $\mathcal{H}$ par rapport à l'origine.
</Callout>
...
<Callout kind="tip" title="Pourquoi diviser par la norme de w">
  Si tu doubles $w$ et $b$ en même temps, l'hyperplan ne change pas géométriquement, mais la
  quantité $w \cdot x + b$ est multipliée par deux. Pour avoir une distance qui ne dépend que de
  l'hyperplan, pas du codage particulier $(w, b)$ qu'on a choisi, on normalise par $\|w\|$.
</Callout>

### Joue avec l'hyperplan

<HyperplaneGeometry client:visible locale="fr" />
...
Frank Rosenblatt publie l'article fondateur dans la revue _Psychological Review_ (Rosenblatt, 1958). Il y propose le modèle théorique et la règle d'apprentissage que tu vas voir tout de suite. Deux ans plus tard, il construit au Cornell Aeronautical Laboratory le <Term slug="mark-i-perceptron">Mark I Perceptron</Term> : une machine physique avec 400 photorécepteurs et des poids ajustables via des potentiomètres motorisés. Le New York Times titre alors que la marine américaine vient de construire « une machine qui apprend par elle-même ».
...
La réponse, étonnamment, est qu'il n'a pas eu besoin d'une dérivée. Sa procédure d'apprentissage est une _correction géométrique locale_ : quand le perceptron se trompe sur un exemple, on déplace le vecteur de poids dans la direction qui corrigerait l'erreur, sans jamais calculer de gradient.
...
<Callout kind="tip" title="Lis l'équation à voix haute">
  *« Le nouveau vecteur de poids w est l'ancien w plus le pas eta multiplié par la cible y i,
  multipliée par l'exemple x i. Le nouveau biais b est l'ancien plus eta fois y i. »* L'update est
  donc une *addition* (ou soustraction) du vecteur $x_i$ au vecteur $w$, selon le signe de $y_i$.
</Callout>
...
<PerceptronGeometricStepper client:visible locale="fr" />
...
- Le taux $\eta$ ne change pas la convergence sur les datasets séparables : il modifie seulement l'amplitude de chaque pas et donc la _vitesse_ visuelle de la frontière, pas le résultat final.
...
<Callout kind="tip" title="Astuce du biais absorbé">
  Pour alléger la rédaction, on absorbe le biais dans les poids : on remplace chaque $x_i \in
  \mathbb{R}^n$ par $\tilde x_i = (x_i, 1) \in \mathbb{R}^{n + 1}$ et chaque $(w, b)$ par $\tilde w
  = (w, b) \in \mathbb{R}^{n + 1}$. La règle d'apprentissage et l'inégalité de séparation gardent
  exactement leur forme, et on peut désormais travailler comme si le biais n'existait pas, dans un
  espace de dimension $n+1$. L'exercice 4 montre pourquoi on ne peut pas simplement *figer* $b = 0$
  : sans cette absorption, la convergence n'est plus garantie.
</Callout>
...
**Lemme 1 (minoration).** _Pour tout $t \geq 0$, $w_t \cdot w^_ \geq t \gamma$.\*
...
**Lemme 2 (majoration).** _Pour tout $t \geq 0$, $\|w_t\|^2 \leq t R^2$._
...
- Mais quelle que soit la difficulté, la borne reste _finie_ tant que $\gamma > 0$.

### Explore la borne en direct

<NovikoffBoundExplorer client:visible locale="fr" />
...
La borne $T \leq R^2 / \gamma^2$ est démontrée sous l'hypothèse $\gamma > 0$. Quand le dataset n'est pas linéairement séparable, $\gamma$ n'est pas défini : il n'existe aucun couple $(w, b)$ qui classe tout correctement. Conséquence : la règle d'apprentissage de Rosenblatt continue à corriger en boucle, sans jamais converger. Le vecteur de poids $w$ oscille indéfiniment, et même les itérations qui passent par un _« presque bon »_ $(w, b)$ sont perdues à l'itération suivante quand un autre exemple mal classé déclenche un update qui dégrade ce qu'on avait.
...
<Callout
  kind="note"
  title="Pourquoi le perceptron pur ne se contente pas de la « moyenne des poids »"
>
  Une idée naïve : faire converger en moyennant les $w_t$ obtenus à chaque itération. Cette
  stratégie marche aussi (c'est le *voted perceptron* de Freund et Schapire (1999)), mais elle exige
  de garder un historique et de pondérer chaque $w_t$ par sa durée de vie. Le Pocket Algorithm est
  plus simple : un seul $(w, b)$ stocké, mis à jour seulement quand on fait strictement mieux.
</Callout>
...
<XorContradictionVisualizer client:visible locale="fr" />
...
**Étape 3.** Vérifions que ce dataset est linéairement séparable _avec biais_. Prenons $(w, b) = (0{,}1, \; 1)$. Pour le premier point : $w \cdot 1 + b = 0{,}1 + 1 = 1{,}1 > 0$, donc prédit $+1$. Pour le second : $w \cdot (-1) + b = -0{,}1 + 1 = 0{,}9 > 0$, donc prédit $+1$ aussi. Les deux points sont bien classés.
  ...
  questions={[
    {
      ...
      answer: 1,
      explain:
        "La règle w ← w + η y x est une projection : on ajoute (ou retranche) le vecteur d'entrée. Elle améliore strictement la marge fonctionnelle de l'exemple corrigé, comme on l'a démontré, sans jamais évaluer la pente d'une fonction.",
    },
    {
      ...
      answer: 1,
      explain:
        "Le théorème de Novikoff ne s'applique pas. À chaque correction, un autre exemple devient mal classé, et l'algorithme passe son temps à corriger l'un puis l'autre sans jamais converger.",
    },
    {
      ...
      answer: 1,
      explain:
        "Si xᵢ = 0, le poids wᵢ n'a pas influencé la prédiction (multiplié par zéro). Le corriger serait arbitraire. La règle ajuste seulement les poids responsables, en proportion de leur entrée.",
    },
    {
      ...
      answer: 1,
      explain:
        "L'hyperplan passe par les points x tels que w · x = -b. La distance signée de l'origine à l'hyperplan est b / ‖w‖. Sans biais, l'hyperplan est forcé de passer par l'origine, ce qui restreint fortement la classe de classifieurs réalisables.",
    },
    {
      ...
      answer: 1,
      explain:
        "On a démontré dans ce chapitre que les quatre contraintes XOR sont incompatibles. Mais la décomposition XOR = OR ∧ ¬AND est faite uniquement de fonctions linéairement séparables. Trois perceptrons empilés résolvent XOR : c'est l'idée du chapitre 5.",
    },
  ]}
/>
...
- Gallant, S. I. (1990). « Perceptron-Based Learning Algorithms ». _IEEE Transactions on Neural Networks_ 1(2), 179-191. (Introduction du Pocket Algorithm.) [DOI 10.1109/72.80230](https://doi.org/10.1109/72.80230)
...
- Cortes, C. & Vapnik, V. (1995). « Support-Vector Networks ». _Machine Learning_ 20(3), 273-297. Les SVM sont précisément le perceptron à **marge maximale** : là où la règle de Rosenblatt accepte n'importe quel séparateur valide, SVM optimise le séparateur dont la marge géométrique $\gamma$ (celle-là même qui apparaît dans Novikoff) est la plus grande possible. C'est l'héritage direct du perceptron, et la pierre angulaire de l'apprentissage supervisé pré-2012. [DOI 10.1007/BF00994018](https://doi.org/10.1007/BF00994018)
- Freund, Y. & Schapire, R. E. (1999). « Large Margin Classification Using the Perceptron Algorithm ». _Machine Learning_ 37(3), 277-296. Introduit le _voted perceptron_ qui agrège les hypothèses intermédiaires : sur dataset bruité, performance proche de SVM pour un coût comparable au perceptron. [DOI 10.1023/A:1007662407062](https://doi.org/10.1023/A:1007662407062)
